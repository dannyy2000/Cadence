...
- [x] Fix 5: deterministic CLVR tie-breaking rule — revised after finding a real exploit in the first version. That version broke ties by arrival order (lower queue index wins) - deterministic, but gameable: an attacker can force an exact tie against a victim by submitting a trade of the identical size and direction, and since front-running means arriving first by definition, arrival-order tie-breaking handed the tie to the attacker every time. Verified empirically, not just reasoned about, in `SandwichDemo.t.sol`: a size-matched front-run produced profit numerically identical to an unprotected plain pool. Fixed by keying the tie-break on `blockhash(block.number - 1)`, captured fresh at settlement time instead of anything fixed at order-submission time - still fully deterministic and checkable after the fact, but no longer predictable in advance by whoever is deciding whether to attempt the attack. Honest result after the fix, also verified empirically: this converts a *guaranteed* exploit into an *unreliable* one, not a fully eliminated one for this specific contrived case (attacker and victim trading the identical size) - across different possible settlement blocks the outcome now genuinely varies, including the attacker landing at a **loss**, but it can still occasionally land in the attacker's favor by chance. See `testSandwich_CadencePool_OutcomeVariesAcrossSettlementBlocks`. A known residual limitation is documented in code: blockhash is unavailable beyond the last 256 blocks, which would degrade the tie-break back to a fixed function of queue position on a batch left overdue that long - an edge case the fallback settlement trigger exists to keep rare.

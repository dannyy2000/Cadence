...
- [x] Invariant tests on settlement *pricing* — `invariant_SettlementPriceStaysWithinSanityBandOfReference` in `CadenceHookInvariant.t.sol`: the handler captures the pool's price immediately before and after every settlement it observes (both triggers - primary and fallback), and the invariant checks the post-settlement price never strays implausibly far from that same settlement's own reference price. Deliberately a generous sanity band (4x in sqrtPrice terms), not a tight replica of CLVR's formal optimality proof - sized so it would only ever fail on a real bug (wrong swap direction, a sign error, a corrupted price read), not on legitimate variance, given the realistic swap sizes the fuzzer can generate. Verified at the full 128,000-call stress level (256 runs × 500 calls), 0 reverts.
...
- [x] `batchWindowBlocks` justification — this one had never been researched at all, just picked as a reasonable-sounding round number; caught when explaining it out loud surfaced that gap. Looked into Budish, Cramton & Shim [2] specifically (the frequent-batch-auctions paper batching is already built on): their own finding is that even a 100ms-1s interval already captures the full benefit of batching over continuous trading, since what removes the harmful racing dynamic is that time is discretized at all, not how long each step is - by that logic, Cadence's core anti-sandwich guarantee doesn't need a long window, since one order alone in its own batch is already just as unsandwichable as several together. Cross-checked against CoW Protocol's [5] real production batches (~30 seconds) - confirmed via their own docs, not assumed - and found a real, specific reason CoW's interval is longer than ours needs to be: it exists to give their off-chain competing solver network time to compute routes, a step CLVR has no equivalent of (it's one fast, deterministic on-chain calculation, not a competition to wait on). Documented honestly in the README, including the real trade-off this doesn't erase: a short window means fewer batches will happen to contain more than one large order, so CLVR's *extra* multi-order fairness benefit (distinct from the core anti-sandwich guarantee, which doesn't need it) shows up less often in practice.
...
- [x] Broad test suite expansion — access control (`onlyPoolManager`, all three self-call guards including the two merge-path externals), multi-pool isolation (two pools on one hook never cross-contaminate queues/deadlines/order-splitting windows/size-cap triggers), more CLVR worked examples (n=1, n=2, n=3+ exact-tie merges, all-same-direction smallest-to-largest per the paper's Section 5.7 — both hand-derived and fuzzed, direction symmetry), deadline-boundary precision (one block before vs. exactly at), multiple consecutive batches, constructor-parameter fuzzing (threshold/window/max-batch-size all independently fuzzed, including the two zero-value revert paths fuzzed against arbitrary other params), configuration boundaries (threshold=0, threshold=max, maxBatchSize=1, maxBatchSize=2 combined with an exact tie), hookData edge cases (wrong length, empty, zero address, arbitrary fuzzed address), event-field verification (including per-member event correctness for a merged settlement, and BatchSettled firing exactly once regardless of how many merge groups a batch contained), and a second reentrancy attack shape (nested `settle()` vs. nested `swap()` — rejected by `PoolManager` itself, not `CadenceHook`, but proven caught cleanly by Fix 2 either way). Also closed two previously-untested merge-path failure modes: a blacklisted recipient *inside* a merged tie group falling back to an ERC-6909 claim mint, and pro-rata split correctness (no wei stranded, remainder bounded) fuzzed across 2–10-way tie groups. **102 tests total** (was 68), including a 4th invariant (`invariant_SettlementPriceStaysWithinSanityBandOfReference`, see above) and `testSandwich_MinimumProfitableSizeSweep`, the trade-size sweep that derived the real `batchThreshold` calibration below. Full suite re-verified green end-to-end the same day as this note — 102/102 passing, 0 failed, 0 skipped; all 4 invariants at 128,000 calls each, 0 reverts
...
- [x] Team section + final README pass — README's Team section filled in (Akinsanya Daniel / dannyy2000, sole builder).
- [x] Final test/security pass — full suite re-run end-to-end same day as this note: 102/102 tests passing (0 failed, 0 skipped), all 4 invariants clean at 128,000 randomized calls each (0 reverts), frontend build clean, live site returning 200. No regressions from the day's expansion.
